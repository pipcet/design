...
A module can declare a sequence of **intrinsic functions** which are defined in the module, but may have differing and potentially more efficient implementations in the host environment. Each such function *must* have a definition in the module, but it is up to the host environment whether this definition or a built-in one is used.
...
Each intrinsic function declaration includes two well-known names: a *library name* and an *export name*. In addition, it contains a function signature. Both the names and the function signatures must match the central register to avoid conflicts.
...
The *function index space* indexes all intrinsic, imported, and
...
Thus, the index space starts at zero with the intrinsic definitions,
followed by the function imports (if any), followed by the functions
